...
import numpy as np
import pandas as pd
import regenerate_vecm_regime_heatmaps_april2023 as plots


def main():
    source = Path("hasbrouck_spec_audit/random_10_days_month_full_grid")
    plots.OUT = source / "regime_heatmaps"
    ...
    summary.to_csv(plots.OUT / "spot_ils_summary.csv", index=False)
    perspectives.to_csv(plots.OUT / "spot_perp_ils_summary.csv", index=False)
    data.groupby(["market", "latency"])["interval"].agg(
        first_interval="min", last_interval="max", model_intervals="nunique"
    ).reset_index().to_csv(plots.OUT / "input_coverage.csv", index=False)
    ...
    (plots.OUT / "README.md").write_text(
    ...
    print(f"Created 8 PNGs, 8 PDFs and summary tables in {plots.OUT}")
