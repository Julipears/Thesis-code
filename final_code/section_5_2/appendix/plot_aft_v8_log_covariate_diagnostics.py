...
def write_distribution_outputs(samples: dict, output: Path) -> None:
    """Write the distribution summary used for the appendix covariate table."""
    output.mkdir(parents=True, exist_ok=True)
    rows = []
    for (market, first), frame in samples.items():
        for covariate in COVARIATES:
            values = pd.to_numeric(frame[covariate], errors="coerce").replace(
    ...
    summary.to_csv(output / "covariate_distribution_summary.csv", index=False)
    latex = [
        r"\begin{tabular}{llrrrr}",
        r"\hline",
        r"Market & Origin & Covariate & Mean & Median & Std. dev. \\",
        r"\hline",
    ]
    for row in summary.itertuples(index=False):
        latex.append(
            f"{row.market.upper()} & {row.first} & {row.covariate} & "
            f"{row.mean:.4g} & {row.median:.4g} & {row.std:.4g} " + r"\\"
        )
    latex.extend([r"\hline", r"\end{tabular}"])
    (output / "covariate_distribution_summary.tex").write_text(
        "\n".join(latex) + "\n", encoding="utf-8"
    )
...
def main() -> None:
    ...
    write_distribution_outputs(samples, OUTPUT)
