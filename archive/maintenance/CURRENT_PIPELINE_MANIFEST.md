...
The runnable source is now organized under `final_code/`: shared modules are
at its root, VECM code is in `section_4/`, KM code is in
`section_5_1/`, and AFT code is in `section_5_2/`. Run modules from the
repository root with `python -m final_code.<section>.<module>`.
Appendix-only generators live in `final_code/section_4/appendix/` and
`final_code/section_5_2/appendix/`, beside the methods they extend.
...
- `run_hasbrouck3_full.py`: BTC/ETH UM/CM production runner.

The runner writes the four saved result folders with the same Hasbrouck-3
...
- `vecm_plotting.py`: alpha-over-time plots, separate alpha plots, and
  April-2023 regime heatmaps.
...
- `fit_aft_v8_univariate_models.py`: shock-size, spread, log-volume, basis,
  and null fits.
- `survival_analysis_pipeline_final.py`: shared event loading and the
  Weibull/lognormal/log-logistic family fitting used for model selection.
- `survival_analysis_utils_final.py`: atomic result writing.

The stopped experimental file is retained at
`archive/legacy_aft/fit_aft_v8_bivariate_shock_spread.py`; its partial output
is not part of the current results.
...
- `plot_aft_v8_log_covariate_results.py`: shared plotting helper used by the
  reduced-specification wrapper.
...
All superseded source files are retained under `archive/` rather than deleted;
see `archive/README.md` for the categories and restoration note.
